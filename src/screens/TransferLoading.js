...
const txnHistoryObject = (accountNumber, query, activeFilter) => ({
  accountNumber,
  url:
    (activeFilter === "all" ? null : !activeFilter ? null : "&category=" + activeFilter) || query,
});
...
function TransferLoading() {
  ...
  useInterval(async () => {
    await getTransactionHistory(txnHistoryObject(accountNumber, null, "all"));
  }, 8000);
