...
export default {
	...
	magicSignup: (data, options) =>
		ApiHandler.post('core/magic/signup', data, options),
	resendVerificationToken: (data) =>
		ApiHandler.post(`${authURL}/basic/auth/account/verification/resend`, data),
	verifyAccount: (data) =>
		ApiHandler.get(
			`${authURL}/basic/auth/account/verification/${Base64.btoa(data)}`
		),
	...
	linkMobileNumber: (data) =>
		ApiHandler.post(`${authURL}/basic/users/link-mobile`, data),
	phoneOTP: (data, options) =>
		ApiHandler.post(`${socialUrl}core/otp/send`, data, options),
	verifyPhoneOTP: (data, options) =>
		ApiHandler.post(`${socialUrl}core/otp/verify`, data, options),
	checkEmail: (data) =>
		ApiHandler.get(`${authURL}/basic/users/email-check/${data}`),
	initPasswordReset: (data) =>
		ApiHandler.post(`${authURL}/basic/password/reset/init?otpMode=email`, data),
	resetPassword: (data) =>
		ApiHandler.post(`${authURL}/basic/password/reset`, data),
	checkUsername: (data) =>
		ApiHandler.get(`${authURL}/basic/users/username-check/${data}`),
	updateProfile: (data) =>
		ApiHandler.post(`${authURL}/core/profile`, data, { appChannel: 'RAPID' }),
	registerPushTokens: (data) =>
		ApiHandler.post(`messaging/core/users/push-token`, data),
};
